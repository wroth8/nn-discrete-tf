...
class QuaternaryWeights(WeightType):
    ...
    def initialize_weights(self, shape, initializer_logits='uniform'):
        ...
        if isinstance(initializer_logits, WeightType):
            ...
            w_expect[idx_neg] = map_to_ecdf(w_expect[idx_neg]) * (4.0 / 3.0) - (4.0 / 3.0)
            w_expect[idx_pos] = map_to_ecdf(w_expect[idx_pos]) * (4.0 / 3.0)
            q_values = initialize_probabilities_from_expectation(w_expect, [-1.0, -1.0 / 3.0, 1.0 / 3.0, 1.0])
        ...
        self.q_logits = tf.Variable(np.log(q_values), trainable=True, name='QuaternaryWeightLogits', constraint=constraint_fun)
        ...
        if self.enable_sampled_weights:
            self.w_sampled = tf.Variable(np.zeros(self.shape[:-1]), trainable=False, name='QuaternaryWeightsSampled', dtype=self.q_logits.dtype)
    ...
    def most_probable(self):
        w_mp = (tf.cast(tf.math.argmax(self.q_logits, axis=-1), self.q_logits.dtype) - 1.5) * (2.0 / 3.0)
        return w_mp
    ...
    def resample_weights(self):
        ...
        w_resampled = (tf.cast(tf.random.categorical(tf.reshape(self.q_logits, [-1, 4]), 1), self.q_logits.dtype) - 1.5) * (2.0 / 3.0)
