...
def downloadCifar10(filename):
    ...
    x_tr = np.concatenate((np.stack(data_batch_1[b'data'], axis=0),
                           np.stack(data_batch_2[b'data'], axis=0),
                           np.stack(data_batch_3[b'data'], axis=0),
                           np.stack(data_batch_4[b'data'], axis=0),
                           np.stack(data_batch_5[b'data'], axis=0)[:5000]), axis=0)
    t_tr = np.concatenate((np.asarray(data_batch_1[b'labels']),
                           np.asarray(data_batch_2[b'labels']),
                           np.asarray(data_batch_3[b'labels']),
                           np.asarray(data_batch_4[b'labels']),
                           np.asarray(data_batch_5[b'labels'])[:5000]), axis=0)
    x_va = np.stack(data_batch_5[b'data'], axis=0)[5000:]
    t_va = np.asarray(data_batch_5[b'labels'])[5000:]
    x_te = np.stack(data_test[b'data'], axis=0)
    t_te = np.asarray(data_test[b'labels'])
