...
create_dirs svhn $EXPERIMENT_DIR
(python experiment_pretrain_real.py \
        --taskid=$TASK_ID \
        --experiment-dir=results/svhn/$EXPERIMENT_DIR \
        --dataset-file=datasets/svhn.npz \
        --dataset=svhn \
        --activation=relu \
        --n-epochs=300 \
        --batch-size=250 \
        ...
        --batchnorm-momentum=0.99 \
            | tee results/svhn/$EXPERIMENT_DIR/logs/log$TASK_ID_AUX.out) 3>&1 1>&2 2>&3 \
            | tee results/svhn/$EXPERIMENT_DIR/logs/log$TASK_ID_AUX.err
...
(python experiment_discrete.py \
        --taskid=$TASK_ID \
        --experiment-dir=results/svhn/$EXPERIMENT_DIR \
        --init-model-file=results/svhn/pretrain_relu/models/model_best_svhn_${TASK_ID_AUX} \
        --dataset-file=datasets/svhn.npz \
        --dataset=svhn \
        --weight-type=ternary \
        --activation=sign \
...
(python experiment_evaluate_sampling.py \
        --taskid=$TASK_ID \
        --experiment-dir=results/svhn/sampling_ternary_sign_lrt_gumbel_softmax \
        --init-model-file=results/svhn/train_ternary_sign_lrt_gumbel_softmax/models/model_best_svhn_${TASK_ID_AUX} \
        --dataset-file=datasets/svhn.npz \
        --dataset=svhn \
        --n-max-samples=100 \
        --n-experiments=10 \
