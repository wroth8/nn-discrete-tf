...
(python experiment_pretrain_real.py \
        ...
        --batchnorm-momentum=0.99 \
            | tee results/mnist/$EXPERIMENT_DIR/logs/log$TASK_ID_AUX.out) 3>&1 1>&2 2>&3 \
            | tee results/mnist/$EXPERIMENT_DIR/logs/log$TASK_ID_AUX.err
...
(python experiment_evaluate_sampling.py \
        --taskid=$TASK_ID \
        --experiment-dir=results/mnist/sampling_ternary_sign_lrt_gumbel_softmax \
        --init-model-file=results/mnist/train_ternary_sign_lrt_gumbel_softmax/models/model_best_mnist_${TASK_ID_AUX} \
        --dataset-file=datasets/mnist.npz \
        --dataset=mnist \
        --n-max-samples=100 \
        --n-experiments=10 \
