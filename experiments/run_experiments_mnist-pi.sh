...
create_dirs mnist-pi $EXPERIMENT_DIR
(python experiment_pretrain_real.py \
        --taskid=$TASK_ID \
        --experiment-dir=results/mnist-pi/$EXPERIMENT_DIR \
        ...
        --batchnorm-momentum=0.99 \
            | tee results/mnist-pi/$EXPERIMENT_DIR/logs/log$TASK_ID_AUX.out) 3>&1 1>&2 2>&3 \
            | tee results/mnist-pi/$EXPERIMENT_DIR/logs/log$TASK_ID_AUX.err
...
(python experiment_discrete.py \
        --taskid=$TASK_ID \
        --experiment-dir=results/mnist-pi/$EXPERIMENT_DIR \
        --init-model-file=results/mnist-pi/pretrain_relu/models/model_best_mnist-pi_${TASK_ID_AUX} \
        --dataset-file=datasets/mnist.npz \
        --dataset=mnist-pi \
        --weight-type=ternary \
        --activation=sign \
...
(python experiment_evaluate_sampling.py \
        --taskid=$TASK_ID \
        --experiment-dir=results/mnist-pi/sampling_ternary_sign_lrt_gumbel_softmax \
        --init-model-file=results/mnist-pi/train_ternary_sign_lrt_gumbel_softmax/models/model_best_mnist-pi_${TASK_ID_AUX} \
        --dataset-file=datasets/mnist.npz \
        --dataset=mnist-pi \
        --n-max-samples=100 \
        --n-experiments=10 \
